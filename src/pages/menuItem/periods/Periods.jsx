  ...
  const [dateBirthday, setDateBirthday] = React.useState("01.01.2020");
		...
		let handleSubmit = async (e, handlePrompt) => {
			...
			switch (isPrompt) {
				...
				case "2":
					data = {
						text: `Как профессиональный нумеролог, составь подробный анализ  жизненных периодов с первого по третий включительно, с учетом данных о человеке его даты рождения и имени, для человека, родившегося ${dateBirthday} По имени ${name}. Распиши детально, но не расписывай формулы с расчётами Опиши плюсы этих периодов, дай рекомендации. Укажи основные точки роста в этих жизненных периодах. (Текущая дата - ${getCurrentDate()})`,
					};
					break;
				case "3":
					data = {
						text: `Как профессиональный нумеролог, составь подробный анализ  жизненных периодов с  четвертого по шестой включительно, с учетом данных о человеке его даты рождения и имени, для человека, родившегося ${dateBirthday} По имени  ${name}. Распиши детально, но не расписывай формулы с расчётами Опиши плюсы этих периодов, дай рекомендации. Укажи основные точки роста в этих жизненных периодах. (Текущая дата - ${getCurrentDate()}`,
					};
					break;
				case "4":
					data = {
						text: `Как профессиональный нумеролог, составь подробный анализ  жизненных периодов с седьмого по девятый включительно, с учетом данных о человеке его даты рождения и имени, для человека, родившегося ${dateBirthday} По имени ${name}. Распиши детально, но не расписывай формулы с расчётами Опиши плюсы этих периодов, дай рекомендации. Укажи основные точки роста в этих жизненных периодах. (Текущая дата - ${getCurrentDate()}`,
					};
					break;
			}
