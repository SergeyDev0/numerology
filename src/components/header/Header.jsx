...
const Header = observer(() => {
  ...
  const shouldShowLink = () => {
    return !["/menu", "/auth", "/registration", "/"].includes(
      location.pathname
    );
  };
  return (
          ...
          <>
            {shouldShowLink() && (
              <div className={styles.headerBackWrapper}>
                <Link
                  to="/menu"
                  className={`${styles.headerBack} ${styles.mobile}`}
                >
                  <img src={arrowIcon} alt="Назад" />
                </Link>
                <div className={`${styles.headerBack} ${styles.desktop}`}>
                  <ButtonOutline url="/menu" text={t("На главную")} />
                </div>
              </div>
            )}
            <nav className={styles.headerNav}>
              <ButtonOutline
                text={languageStore.language === "ru" ? "Eng" : "Rus"}
                onClick={(e) => {
                  e.preventDefault();
